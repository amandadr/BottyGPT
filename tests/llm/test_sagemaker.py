...
class TestSagemakerAPILLM(unittest.TestCase):
    def setUp(self):
        ...
        self.messages = [{"content": self.context}, {"content": "Some other message"}, {"content": self.user_question}]
        ...
        self.response = {"Body": MagicMock()}
        self.result = [{"generated_text": "This is the generated text"}]
    ...
    def test_gen(self):
        with patch("application.cache.get_redis_instance") as mock_make_redis:
            ...
            with patch.object(
                self.sagemaker.runtime, "invoke_endpoint", return_value=self.response
            ) as mock_invoke_endpoint:
                output = self.sagemaker.gen(None, self.messages)
                mock_invoke_endpoint.assert_called_once_with(
                    EndpointName=self.sagemaker.endpoint, ContentType="application/json", Body=self.body_bytes
                )
                self.assertEqual(output, self.result[0]["generated_text"][len(self.prompt) :])
    ...
    def test_gen_stream(self):
        with patch("application.cache.get_redis_instance") as mock_make_redis:
            ...
            with patch.object(
                self.sagemaker.runtime, "invoke_endpoint_with_response_stream", return_value=self.response
            ) as mock_invoke_endpoint:
                output = list(self.sagemaker.gen_stream(None, self.messages, tools=None))
                mock_invoke_endpoint.assert_called_once_with(
                    EndpointName=self.sagemaker.endpoint, ContentType="application/json", Body=self.body_bytes_stream
                )
